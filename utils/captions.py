startup_message = f"""
🤖 Бот успешно запущен!

Доступные команды:
`/list` - показать список чатов.

⚙️ Вы можете отправить сюда файлы:
`chats.txt` - для обновления списка чатов для прослушивания
`words.txt` - для обновления списка слов для прослушивания
`answers.txt` - для обновления списка заготовленных сообщений для отправки
`skip_words.txt` - для обновления списка стоп-слов
"""

spam_message = '**Аккаунт в спаме**'

chats_document_caption = f'''
📄 Ваш файл со всеми ID чатов, которые у вас есть.
Выберите нужные ID и отправьте их в файле chats.txt
'''

chats_list_updated_message = 'Список чатов был успешно обновлён!'

words_list_updated_message = 'Список слов был успешно обновлён!'

stop_words_list_updated_message = 'Список стоп-слов был успешно обновлён!'

answers_list_updated_message = 'Список заготовленных ответов был успешно обновлён!'

account_blocked_message = f'\n---Аккаунт заблокирован---\n'
